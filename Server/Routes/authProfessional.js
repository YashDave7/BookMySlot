...
//     // body('password', 'Enter a 8 character password').isLength({ min: 8 }),
//     // body('fees', 'Enter your average fees').exists(),
//     // body('timing', 'Enter your timing operation').isArray(), // Validate that 'timing' is an array
//     // body('timing.startTime', 'Enter a valid start time').exists(), // Validate each timing object in the array
//     // body('timing.endTime', 'Enter a valid end time').exists(),
//     // body('timing.eachSlotTime', 'Enter a valid total slot time').exists(),
// ], async (req, res) => {
//     // If there are errors, return the bad request and the errors.
//     const errors = validationResult(req);
//     if (!errors.isEmpty()) {
...
//             timingsData = req.body.timings.map(time => ({ timeFrame: time.timeFrame }));
//         }
